...
Before developing an operator, install the driver, firmware, CANN Toolkit, and ops operator package. For details, see [CANN Software Installation Guide](https://www.hiascend.com/cann/download). No installation example is provided in this section.
...
- Cache hit ratio
    ...
    L2 cache refers to the high-bandwidth transfer channel between part of the GM space and the vector core and the cube core. When the L2 cache hit ratio is close to 100% compared to when it is near 0%, there can be more than a twofold difference in bandwidth. Currently, msKPP allows users to manually adjust the L2 cache hit ratio.
...
- Debug mode
    ...
    This mode enables users to preliminarily locate the instructions that encounter dequeue and enqueue issues during operator modeling, thereby enhancing the efficiency of joint fault location during tool development. The enabling method is as follows:
...
- Comparison between theoretical values of pipeline information and values measured by msprof
    ...
    The following uses an Ascend C operator as an example. You can use the `--application` mode to call msprof, and output the `PipeUtilization.csv` file to the `OPPROF_{timestamp}_XXX` directory.
    ...
    **Figure 1** Pipe_statistic.csv file
    ...
    ![](../figures/Pipe_statistic-csv.png "pipe_statistic_csv_file")
...
|Size(Byte)|Transfer volume of a transfer pipeline (unit: Byte).|
|Ops|Size of a calculation element in pipelines of the calculation class.|

For the pipeline that takes the longest time and is clearly a bottleneck in transfer performance, the optimization roadmap is as follows:
...
**Figure 3** Instruction_statistic.csv
...
Click the `MOV-GM_TO_L1` instruction in the pipeline to view the number of cycles and bandwidth of the instruction under the current transfer volume and calculation volume, as shown in [Figure 5](#222).

**Figure 5** Instruction details <a name="222"></a>
...
**Figure 6** Instruction duration statistics
